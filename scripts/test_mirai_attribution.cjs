...
const source = fs.readFileSync('cosmetics/public/js/tracking.js', 'utf8');
const links = [
...
assert.equal(actual.searchParams.get('utm_source'),'glow-coded.com');
...
assert.equal(new URL(dynamic.href).searchParams.get('utm_source'),'glow-coded.com');
